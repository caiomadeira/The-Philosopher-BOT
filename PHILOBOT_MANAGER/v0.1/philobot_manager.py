...
"""
import tweepy
from Credentials.credentials_verify_test import api as api_teste
from Credentials.credentials_verify import api as api_oficial
from Credentials.credentials_verify_reserva import api as api_extension
from config import PATH_FILES_LOGIN
...
def info_accounts():
    ...
    if question == "1":
        path = PATH_FILES_LOGIN
        f = open(f"twitter_accounts_logins.txt", "r")
        print(f.read())
    if question == "2":
        path = PATH_FILES_LOGIN
        f = open(f"heroku_accounts_logins.txt", "r")
        print(f.read())
    else:
        retornar_question()
